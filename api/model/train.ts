 ...
 *
 * Called automatically by /api/verify for every human-classified visitor.
 */
...
// No external dependencies — runs inside Vercel Edge/Node runtime
// ─────────────────────────────────────────────────────────────────────────────
...
function trainStep(
  ...
  for (let j = 0; j < 2; j++)
    for (let k = 0; k < 4; k++)
      weights2[j][k] -= LEARNING_RATE * dOut[k] * hidden[j];
  for (let k = 0; k < 4; k++)
    bias2[k] -= LEARNING_RATE * dOut[k];

  // Update weights1 and bias1
  for (let i = 0; i < 4; i++)
    for (let j = 0; j < 2; j++)
      weights1[i][j] -= LEARNING_RATE * dHid[j] * input[i];
  for (let j = 0; j < 2; j++)
    bias1[j] -= LEARNING_RATE * dHid[j];

  return { weights1, weights2, bias1, bias2, loss };
}

// ─────────────────────────────────────────────────────────────────────────────
// Default initial weights  (Xavier / small random)
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// Feature extraction from raw behavior payload
// ─────────────────────────────────────────────────────────────────────────────
function extractFeatures(behavior: {
  ...
  // 1. Mouse straightness  (1.0 = perfectly straight, >1 = curved)
  ...
  // 2. Typing standard deviation (ms), normalised to [0,1]
  let typingSd = 0;
  if (timings.length >= 2) {
  ...
  // 3. Velocity entropy of mouse path
  let entropy = 0;
  if (pts.length >= 3) {
    const speeds: number[] = [];
    for (let i = 1; i < pts.length; i++) {
    ...
    }, 0) / 3; // max entropy ~3 bits → normalise
  }

  // 4. Interaction duration, normalised (cap at 60 s)
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (label !== 'human') {
      return res.status(200).json({ skipped: true, reason: 'Non-human label — skipping training.' });
    }

    // ── Load current weights from DB ──────────────────────────────────────────
    let row = await loadLatestWeights();
    ...
    // ── If caller sent pre-deployed weights (from frontend), just persist them
    if (inW1 && inB1 && inB2) {
      await saveWeights(inW1, inW1 /* placeholder w2 */ , inB1, inB2, inCount ?? count);
      return res.status(200).json({ success: true, message: 'Weights persisted from frontend deploy.' });
    }

    // ── Build input feature vector ────────────────────────────────────────────
    let input: number[];
    if (mousePoints || keyTimings || formDuration !== undefined) {
      // Raw behavior → extract
      ...
      return res.status(400).json({ error: 'Provide either raw behavior payload or pre-computed feature values.' });
    }

    // ── Run SGD training steps ────────────────────────────────────────────────
    let loss = 0;
    for (let e = 0; e < Math.min(epochs, 200); e++) {
      ({ weights1: w1, weights2: w2, bias1: b1, bias2: b2, loss } =
        trainStep(input, w1, b1, w2, b2));
    }
    count++;
